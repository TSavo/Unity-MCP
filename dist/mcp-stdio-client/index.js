...
exports.createMcpStdioClient = createMcpStdioClient;
 ...
 */
async function createMcpStdioClient(webServerUrl = process.env.WEB_SERVER_URL || "http://localhost:8080") {
    ...
    server.tool("execute_code", {
        code: zod_1.z.string().describe("C# code to execute in Unity"),
        timeout: zod_1.z.number().optional().describe("Timeout in milliseconds (default: 1000)")
    }, async ({ code, timeout = 1000 }) => {
        try {
            logger.info(`Executing code: ${code.substring(0, 100)}${code.length > 100 ? '...' : ''}`);
            // Forward to Web Server
            const response = await axios_1.default.post(`${webServerUrl}/tools`, {
    ...
    // Query tool
    server.tool("query", {
        query: zod_1.z.string().describe("Query using dot notation to access objects and properties"),
        timeout: zod_1.z.number().optional().describe("Timeout in milliseconds (default: 1000)")
    ...
    server.tool("get_result", {
        logId: zod_1.z.string().describe("Log ID of the result to retrieve")
    }, async ({ logId }) => {
        try {
    ...
    server.tool("get_logs", {
        count: zod_1.z.number().optional().describe("Number of logs to retrieve (default: 10)"),
        offset: zod_1.z.number().optional().describe("Offset for pagination (default: 0)")
    }, async ({ count = 10, offset = 0 }) => {
        try {
            logger.info(`Getting logs with count: ${count}, offset: ${offset}`);
            // Forward to Web Server
            const response = await axios_1.default.get(`${webServerUrl}/operations`, {
                params: {
                    limit: count,
                    offset
                }
    ...
    server.tool("get_logs_by_name", {
        log_name: zod_1.z.string().describe("Name of the log to retrieve"),
        limit: zod_1.z.number().optional().describe("Maximum number of log entries to return (default: 10)")
    ...
    server.tool("get_log_details", {
        logId: zod_1.z.string().describe("Log ID of the log entry")
    }, async ({ logId }) => {
        try {
    ...
    server.tool("help", {}, async () => {
        try {
            ...
            // Forward to Web Server
            const response = await axios_1.default.get(`${webServerUrl}/help`);
            const result = response.data;
